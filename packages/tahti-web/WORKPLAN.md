...
- [ ] **SaveButton sweep:** every control that persists an existing entity (`Saving…` / `Save` / `Save changes` / custom save label) must use Storybook `SaveButton` (`Idle` / `Saving` / `Disabled` / `CustomLabel`). Pass `label` / `savingLabel` when the default "Save" is too vague. Do not use it for create/publish/connect/wizard-finish, autosave status text, or icon-only toolbar saves that cannot take a label. Candidate list: [docs/todo/savebutton-statchip-sweep.md](../../docs/todo/savebutton-statchip-sweep.md). Also add a `Disabled` story to `SaveButton.stories.tsx` (Idle/Saving/CustomLabel exist; disabled does not).
- [ ] **StatChip sweep:** artist and other labeled counts (followers, following, likes, plays, tracks, releases, collections, subscribers, listeners) use Storybook `StatChip` (value + uppercase label, optional icon), matching the player `ArtistSocialHeader`. `StatNumber` stays for hero accent figures (grant €, chart-header totals). Compact icon+number overlays on artwork/player (like/comment buttons) stay as they are. Candidate list in the same todo.
