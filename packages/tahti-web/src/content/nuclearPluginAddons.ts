...
export const NUCLEAR_PLUGIN_ADDONS: NuclearPluginAddon[] = [
  ...
  },
  {
    ...
    statusLabel: 'Embeds available',
    fields: [],
    note: 'The YouTube listener widget already plays a full playlist inline (paste a youtube.com/playlist?list=… or watch?v=…&list=… URL) using the official embedded player. Importing playlist entries into the Tahti library for offline/rights-aware playback is not implemented.',
    apiCounterpart: {
      status: 'partial',
      routes: ['/api/v1/channels/:slug/disco-widgets'],
      note: 'Embedded playlist playback is supported; a server-side import/rights contract for library ingestion is not.',
    },
    realFeature: {
      kind: 'plugin-category',
      category: 'listen',
      label: 'Open the real YouTube playlist widget in Listen',
    },
  },
  {
    id: 'omnisource',
  ...
  },
  {
    ...
    statusLabel: 'API contract needed',
    fields: [],
    note: 'Unlike SoundCloud, Bandcamp has no public embeddable profile/feed widget — only its per-track/per-album EmbeddedPlayer (already available in the Listen add-on for a specific release). Album of the Day, New & Notable, and Bandcamp Weekly have no public embed or API Tahti can use honestly, so this stays unimplemented rather than faking a feed.',
    apiCounterpart: {
      status: 'missing',
      routes: [],
  ...
  },
  {
    ...
    status: 'partial',
    statusLabel: 'Profile embed available',
    fields: [],
    note: "The SoundCloud listener widget accepts an artist's public profile URL and embeds a rolling feed of their public tracks using SoundCloud's own widget — a real, honest stand-in for a personal dashboard. SoundCloud has no public charts/editorial-picks or personalized-recommendations embed, so that half of this entry remains unavailable.",
    apiCounterpart: {
      status: 'partial',
      routes: ['/api/v1/channels/:slug/disco-widgets'],
      note: 'Profile-feed embedding is supported; charts, editorial picks, and personalized recommendations are not exposed by SoundCloud for public embedding.',
    },
    realFeature: {
      kind: 'plugin-category',
      category: 'listen',
      label: 'Open the real SoundCloud profile widget in Listen',
    },
  },
];
