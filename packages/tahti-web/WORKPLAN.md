...
- [ ] **System rule — URL-field copy convention:** every field that displays a URL (a share link, smartlink, RTMP server URL, embed src, etc.) must show it in a visible `<code>`/text field and pair it with `@tahti-player/ui`'s `CopyButton` inline — never a bare copy-icon button with no visible URL, and never a hand-rolled copy handler when `CopyButton` already does it. Established as the pattern in `ChannelShareButton.tsx`/`EmbedButton.tsx`; applied to `SoundShareLinksSection.tsx` (2026-09-02). Known violators to sweep: `StudioReleasesView.tsx`'s "Copy smartlink" icon button has no visible link field at all; `StudioGoLiveView.tsx`'s local `copyText`/labeled-field component (~line 63) hand-rolls the same check-icon-swap `CopyButton` already provides instead of importing it. `ApiTokensPanel.tsx`, `StudioDistributionView.tsx`, and `AdminAgmView.tsx` also call `navigator.clipboard.writeText` directly but copy a token/text block, not a URL — out of scope for this rule, not flagged.
