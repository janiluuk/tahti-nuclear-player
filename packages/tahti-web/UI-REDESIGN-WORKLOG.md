...
list were completed this round — see "Round 3: five slices closed" above;
...
**Radio widgets: wrong action icon, add a real cover-art upload:**
- `PluginItem.tsx` (`packages/ui/src/components/PluginItem/PluginItem.tsx:155-164`) always renders `onViewDetails` as a gear `SettingsIcon`, positioned in its own action column outside the widget's main icon/body row. `RadioCategory` (`PluginStorePanel.tsx:1504-1514`) wires that same `onViewDetails` to `play(...)` for a configured station — so the button that actually previews/plays a station shows a settings-gear icon, not a play icon, and sits away from the rest of the widget. `PluginItem` is shared by every add-on category, so either give it an icon override for this prop, or give radio a dedicated play action instead of reusing `onViewDetails`.
- The station "Configure" form (`PluginStorePanel.tsx:1526-1538`, fields around line 1576) only has a plain `logoUrl` text input for cover art — no upload widget. Add the standard image-upload flow (`FilePicker`/`uploadUserMediaFile`, the same pattern `ImageUploadField.tsx` and `ArtistImagePurposePicker.tsx` already use) with a small icon that toggles a plain URL text field as the alternate input, mirroring the existing `videoUrlOpen` toggle in `ChannelDesigner.tsx` (`selectVideoFile`/`videoUrlOpen` state) rather than inventing a new toggle pattern.

**Front page: show "now playing" per item, and a continue-listening thumbnail:**
- `ListenView.tsx`'s "On air" section (`374-409`) renders live channels as a plain `CardGrid` with no per-card "you're listening to this" state — compare with the single featured Tahti Radio card just above it (`211-216`, `304`, `354`), which already derives `radioIsCurrent`/`radioIsPlaying` from the player store's `currentId`; the same comparison needs to extend to each card in the "On air" grid.
- There's no "continue listening" section anywhere on the front page today — `HistoryView.tsx`/`fetchHistory` already has the data (used by the History tab, `ListenView.tsx:272` route), but nothing surfaces the single most-recent item as a prominent thumbnail on the main Listen page.

**"Your feed": use the standard thumbnail-with-overlay-controls treatment:**
`FeedView.tsx`'s track/release items (`220-289`) render a small 48px square thumbnail plus a title, with `MediaIconActions` play/queue buttons off to the side (`253-268`) — a different, more cramped treatment than the `Card` component's own overlay pattern already used on the front page (`ListenView.tsx`'s `CardGrid`s, e.g. `378-405`: `onPlay`/`onQueue` render as icons overlaid in the middle/corner of the thumbnail itself). Bring the feed's item rendering onto the same `Card`-style thumbnail so play sits centered on the artwork and queue sits in a corner, instead of the current side-by-side row layout.

**Confirm before queueing multiple tracks at once:** `queueAlbum` (`ArtistView.tsx:343`, used at `1221` and `1421`) and `queueArtist` (`ListenView.tsx:196`, used at `481`) both add every track from an album/artist to the play queue in one action with no confirmation — add a confirm step (count of tracks, maybe a preview list) before committing a multi-track enqueue, so a stray click doesn't silently dump a whole discography into the queue.
