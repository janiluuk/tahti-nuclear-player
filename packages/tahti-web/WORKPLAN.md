...
- [ ] **Fan-sub vs track purchase (same original file)** — Playwright `e2e/fan-sub-and-track-purchase.spec.ts` (run 2026-09-03 against mock Vite). Contract: subscriber download and a separate track purchase both yield the uploaded original (`riff.wav`, 16-bit/44.1 kHz stereo), the artist sees those two orders, and the board audit log records them. Currently red; gaps:
  ...
  - [ ] **Same-file / same-quality is not implemented (and the spec disagrees with itself).** The engagement spec serves Opus/MP3 to free listeners and original/FLAC to subscribers; the API only serves FLAC when `?format=flac` and the *artist* is not FREE; Nuclear `fetchPublicArchiveDownload` never passes `format`. Mock download is a third-party MP3 (`DEMO_MP3`), not the upload. Track Download uses an `<a href>` without `download`, so the click may navigate instead of saving.
  - [ ] **Mock uploads are invisible to listeners.** `uploadSoundFile` mints `arch-mock-*` ids; `mockTrackDetail` only reconstructs `${slug}-archive-N`. `/t/:id` after upload has no Download and cannot return `riff.wav`.
  - [ ] **Subscribe does not create an order or audit row.** Mock activate is in-session only. Studio → Audience `mergeRevenueOrders()` lists static fan-sub payouts + Revelator royalties, not this fan and not downloads. `FAN_SUBSCRIPTION_CREATE` in Admin → Logs is a canned DJ Kaski line.
  - [ ] **Artist cannot see the board audit log.** `/admin/logs` is board-gated; patching `isBoard` in localStorage is overwritten by `/me` unless `VITE_MOCK_ADMIN=1`. Downloads are not mapped in `AdminActivityView` at all.
  - [ ] **Upload button name collision.** Top bar and Studio Upload submit both expose accessible name “Upload”.
  - [ ] **Fixture path.** Requested `~/Downloads/riff.wav`; the file on this machine is `~/Music/riff.wav` (RIFF WAVE PCM 16-bit stereo 44100 Hz). Override with `TAHTI_E2E_RIFF_WAV`.
...
- [x] Replace hand-styled native controls in listener and Studio surfaces with Storybook-backed Nuclear components where behavior permits: `Input`, `Select`, and `Textarea` in `ChannelRadioPlaylistPanel`, `RadioBookingCalendar`, `StreamManagerPanel`, `StudioEditorListView`, `StudioDistributionView`, `StudioReleasesView` — re-verified 2026-09-02, all six files now have no native form controls left. A full storybook-vs-`@tahti-player/ui` re-audit the same day found the remaining gaps are elsewhere: `StashFilesPanel`, `AdminI18nView`, `AdminAnnouncementsView`, `AdminAgmView` still hand-roll `Input`/`Select`/`Textarea` (`BroadcastPreflightPanel` and `GlobalSearch` text fields, plus `AdminGovernanceView` board-resolution form, fixed 2026-09-03); `ArtistGalleryPanel`, `StashFilesPanel`, `AdminI18nView`, `StudioBrandingView` hand-roll file inputs instead of `FilePicker`; `AdminStreamManagerPanel`, `WidgetTrackRow`, `ChannelView` hand-roll spinners instead of `Loader` (`StreamManagerPanel` status pill → `Badge` fixed 2026-09-03); remaining native checkboxes vs `Toggle` include `StudioShowsView`, `StudioShowDetailView`, `StudioPlaylistsView`, `StudioBrandingView`, `AddToPlaylistPanel`, `AdminAgmView`, `AdminAnnouncementsView` (`AudienceVisibilitySection`, `StudioCollectionsView`, `StudioSoundView`, `StudioScheduleView` fixed 2026-09-03). Not yet fixed — tracked as a fresh backlog, not this (now-done) line.
