...
This is the smallest useful Nuclear plugin in this repository. It owns one persisted setting, reads that setting when enabled, and uses the lifecycle hooks expected by the plugin loader.

## Try it

From this directory, install the SDK and load the folder through Nuclear’s plugin settings:
...
- The setting ID is bare (`showWelcome`); Nuclear namespaces it as `plugin.nuclear-plugin-example.showWelcome`.
