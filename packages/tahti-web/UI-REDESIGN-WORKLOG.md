...
- Show size in the list itself, and make the list sortable by file type, size, length, and name (no sorting exists today). Show size and type as part of the displayed name/label, not just in a separate column.
- Clicking the artist name on a row should open the same user-info editor modal Admin → Users already uses (`AdminUsersView.tsx`) for that user, rather than linking away to a different page.
...
**Track editor save feedback:**
- Pressing Save in the track editor (`TrackEditDialog.tsx`) should show a toast reporting whether the save succeeded or failed, replacing whatever inline status text currently appears in the form itself (remove that text once the toast covers it).
