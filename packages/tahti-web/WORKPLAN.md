...
- [ ] Audit custom actions against Nuclear `Button`, `FavoriteButton`, `MediaIconActions`, `CopyButton`, and `SaveButton`; prioritize Artist gallery actions, channel layer actions, collection actions, and Radio actions while preserving legitimate custom tab, drag-handle, and row-selection buttons. Progress 2026-09-03 (0.0.44): collection favorite/copy plus shared `ConfirmDialog` on gallery/announcements/stash/tokens/disco-widgets/posts.
- [ ] Normalize remaining loading, empty, error, and status treatments against `PageLoading`, `PageEmpty`, `EmptyState`, `Loader`, and `Badge`. Progress 2026-09-03 (0.0.43): stash / channel chat / add-to-playlist empty copy use `EmptyState`. Image lightbox now uses `Dialog` (0.0.44).
